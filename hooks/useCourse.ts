import { Course, CourseParams, CourseRequest, CourseResponse, fetchCourse } from "@/lib/api/services/fetchCourse"
...
export function useGetCourseByInstructor(filterParams?: CourseParams) {
    const { data, isLoading, refetch, isFetching, isError } = useQuery<CourseResponse, Error, {
        courses: Course[];
        count: number;
        page: number;
        pageSize: number;
