...
import OrderSummary from "../[courseId]/checkout/components/OrderSummary";
import CheckoutForm from "../[courseId]/checkout/components/CheckoutForm";
...
export default function CheckoutPage() {
  ...
  const { courseSummary, isLoading: isLoadingSummary, isError: isErrorSummary } = useGetCourseSummary(courseId || "");
  const { courseDetails, isLoading: isLoadingDetails, isError: isErrorDetails } = useGetCourseDetails(courseId || "");
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-background flex flex-col">
        <Header />
        <div className="flex-1 w-full container mx-auto px-4 py-8"><Skeleton className="h-96 w-full mb-8" /></div>
        <Footer />
      </div>
    );
  }
          ...
          <div className="lg:col-span-8 space-y-8">
            <div className="p-6 md:p-8 rounded-lg border border-brand-magenta/20 shadow-xl shadow-brand-magenta/5 backdrop-blur-xl bg-white/80 dark:bg-black/80">
              <div className="flex flex-row items-center justify-between gap-4 mb-4">
                <Button variant="ghost" onClick={() => router.push(courseUrl(slug, courseId))} className="flex items-center gap-2 text-brand-magenta hover:text-brand-magenta/80 hover:bg-transparent">
                  <ArrowLeft className="w-4 h-4 text-brand-magenta" /><span>Quay lại</span>
                </Button>
                <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-brand-magenta/5 text-brand-magenta font-semibold text-sm border border-brand-magenta/10">
                  <QrCode className="w-4 h-4" /><span>Cổng thanh toán VNPay</span>
                </div>
              </div>
