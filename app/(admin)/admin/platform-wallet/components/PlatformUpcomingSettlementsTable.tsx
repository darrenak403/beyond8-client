...
export function PlatformUpcomingSettlementsTable({
    ...
    const { updateSettlementEligibleAt, isPending: isUpdatingSettlement } = useUpdateSettlementEligibleAt();
    ...
    const handleSubmitSettlement = async (orderId: string) => {
        await updateSettlementEligibleAt({
            ...
            request: {
                note: null,
                settlementEligibleAt: settlementDate ? new Date(settlementDate).toISOString() : new Date().toISOString()
            }
        });
                                        ...
                                        >
                                            ...
                                            <PopoverContent className="w-auto p-4" align="end">
                                                <Calendar
                                                    ...
                                                    onSelect={(date) => {
                                                        if (date) {
                                                            const existing = settlementDate ? new Date(settlementDate) : new Date();
                                                ...
                                                <div className="p-3 border-t border-border mt-3">
                                                    <div className="flex items-center gap-2 mb-3">
                                                        ...
                                                        <Input
                                                            type="time"
                                                            value={settlementDate ? format(new Date(settlementDate), "HH:mm") : "00:00"}
                                                            onChange={(e) => {
