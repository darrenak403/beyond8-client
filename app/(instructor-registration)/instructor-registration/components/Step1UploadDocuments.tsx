...
import { Upload, X, CheckCircle2, CreditCard, Camera, RotateCcw, User } from "lucide-react";
...
export default function Step1UploadDocuments({ data, onChange }: Step1Props) {
  ...
  // Kiểm tra xem đã có ảnh khuôn mặt chưa
  const hasFacePhoto = !!data.facePhoto;
  ...
  const handleFacePhotoUpload = (file: File) => {
    ...
    reader.onloadend = () => {
      ...
      onChange({ ...data, facePhoto: result });
    };
    reader.readAsDataURL(file);
  };
  ...
  const captureFacePhoto = useCallback(() => {
    ...
    if (imageSrc) {
      setFacePhotoPreview(imageSrc);
      onChange({ ...data, facePhoto: imageSrc });
      setShowWebcam(false);
    }
  }, [data, onChange]);
  ...
  return (
    <div className="w-full h-full space-y-6 overflow-y-auto scrollbar-hide">
      {/* Step 1: Face Photo - Pro Max Design */}
      {!hasFacePhoto && (
      ...
      {/* Step 2: CCCD Upload - Only show after face photo is captured */}
      {hasFacePhoto && (
        <div className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          {/* Show face photo summary if already captured */}
          <Card className="border-2 border-green-100 bg-green-50/50">
            <CardContent className="pt-6">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <h3 className="font-semibold text-green-900 mb-1">Ảnh khuôn mặt đã được chụp</h3>
                  <p className="text-sm text-gray-600">Bạn có thể tiếp tục với việc tải lên CCCD</p>
                </div>
                <Button
                  variant="outline"
                  size="sm"
