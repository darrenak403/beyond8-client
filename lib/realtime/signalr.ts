...
function getBaseUrl(): string {
  return process.env.NEXT_PUBLIC_API_URL || "http://api-gateway-beyond8.dev.localhost:8080/";
}
...
export function getHubConnection(): HubConnection {
  ...
  connection = new HubConnectionBuilder()
    ...
    .withAutomaticReconnect({
      // Retry indefinitely: 0s, 2s, 10s, 30s, then every 60s forever
      nextRetryDelayInMilliseconds: (ctx) => {
        ...
        if (ctx.previousRetryCount === 3) return 30000;
        return 60000;
      },
    })
    .configureLogging(
  ...
  connection.onclose((err) => {
    ...
    // Reset singleton so next call to getHubConnection() creates a fresh instance
    connection = null;
    startPromise = null;
  });
